# determine what chi-squared value cut-off to use for spectra
...
n = 30  # for establishing threshold: T = m * n, where m is the median of the medians of pixels that exceed the continuum
        ...
        if True:
            ...
            if pixel_width > 0:
                ...
                if detection_was_injected:
                    ...
                    fig = plt.figure()
                    plt.step(x, y, label = 'Detection')
                    plt.plot(x, gaus2, label = 'Gaussian')
                    plt.title('Chi-squared: ' + str(round(min_chi_squared, 4)))


                    plt.xlabel('Wavelength [A]')
                    plt.ylabel('Flux')
                    plt.legend()
                    path = '/mnt_home/zoek/code/APF-BL-DAP/Zoe/test/'
                    name = str(round(min_chi_squared, 4)) + '.png'
...
np.save('spectra_chi_squared', all_chi_squared_vals)
np.save('spectra_widths', corresponding_widths)